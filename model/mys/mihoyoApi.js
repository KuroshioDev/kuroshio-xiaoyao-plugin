...
const logger = new Logger("xiaoyao-model-miHoYoApi")
...
class miHoYoApi {
  ...
  async init() {
    // //初始化配置文件
    let data = await this.getStoken(this.e.user_id);
    if (data) {
	...
	async getData(type, data = {}) {
		...
		if (!response.ok) {
			logger.error(`[接口][${type}][${this.e.uid}] ${response.status} ${response.statusText}`)
			return false
		}
		...
		if (!res) {
			logger.error('mys接口没有返回')
			return false
		}
    logger.info(`[接口][${type}][${this.e.uid}] ${Date.now() - start}ms-${res.message}`)
		if (res.retcode !== 0) {
			logger.info(`[米游社接口][请求参数] ${url} ${JSON.stringify(param)}`)
		}
		res.api = type
	...
	async getAgent() {
		if (isV3) {
			let cfg = await require(`${common.getPluginsPath()}/lib/config/config.js`);
