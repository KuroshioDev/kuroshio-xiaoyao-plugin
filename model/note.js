...
class note {
	constructor(e) {
		...
		this.Cfg = `${common.getPluginsPath()}/${plugin}/config/`
		this.role_user = Data.readJSON(`${common.getPluginsPath()}/xiaoyao-plugin/resources/dailyNote/json/`, "dispatch_time");
    gsCfg.cpCfg("config", "note")
		this.tempData = Data.readJSON(`${common.getPluginsPath()}/${plugin}/data/NoteTemp`, "tempData")
		this.noteCfg = gsCfg.getfileYaml(this.Cfg, "note") || {}
		this.cfg = gsCfg.getfileYaml(this.Cfg, "config");
	}
	addNote() {
		this.isGroup()
	...
	async getNote(cookie, uid, res,{render}){
		...
		if (this.e.isTask) {
			logger.info(`体力推送:${this.e.user_id}`);
		}
		...
		let img_path = `${common.getPluginsPath()}/xiaoyao-plugin/resources/dailyNote/${path_img[mb]}`;
		...
		await Common.render(`dailyNote/${path_url[mb]}`, {
			...
			coinTime_mb2Day,
			coinTime_mb2,
			urlType: encodeURIComponent(img_path.replace(common.getPluginsPath()+'/xiaoyao-plugin/resources/',"").replace(/\/icon\/bg/g,"")).replace(/%2F/g, "/"),
			resinMaxTime_mb2Day,
			resinMaxTime,
			resinMaxTime_mb2,
			remained_time,
	...
	note_file(xiaoyao) {
		let url1 = `${common.getPluginsPath()}/xiaoyao-plugin/resources/dailyNote/Template/`
		let url2 = `${common.getPluginsPath()}/xiaoyao-plugin/resources/BJT-Templet/` //冤种情况。。
		let url3 = `${common.getPluginsPath()}/xiaoyao-plugin/resources/dailyNote/background_image/`
