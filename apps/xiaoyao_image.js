...
const list = ["wuqi_tujian", "shiwu_tujian", "yuanmo_tujian", "mijin_tujian", "shengyiwu_tujian", "daoju_tujian", "specialty"]
const reglist = ["(#|专武|武器|图鉴)", "(#|食物|特殊料理|特色|料理|食材|图鉴)", "(#|原魔|怪物|图鉴|信息)", "(#|秘境|信息|图鉴)", "(#|圣遗物|图鉴|本|套)",
	"(#|图鉴|道具)", "(#|图鉴|材料)"
]
let pathPlus = `${_path}/${plugin}/resources/xiaoyao-plus/`
async function AtlasAlias(e) {
...
async function getBasicEvent(e) {
	...
	let name, type;
	let list = gsCfg.getfileYaml(`${_path}/${plugin}/resources/Atlas_alias/`, 'Basic_Event')
	name = info_img(e, list, msg)
	if(!name){
		list=gsCfg.getfileYaml(`${_path}/${plugin}/resources/Atlas_alias/`, 'wuqi_tujian')
		name = info_img(e, list, msg)
	}
...
/** 获取角色卡片的原图 */
async function getBasicVoide(e) {
...
async function roleInfo(e) {
	let msg = e.msg.replace(/#|＃|信息|图鉴|命座|天赋|原牌|七圣召唤|七圣|动态|幻影/g, "");
	let Botcfg, id, type = 'juese_tujian';
	Botcfg = require(`${common.getPluginsPath()}/genshin/model/gsCfg.js`);
	...
	if (/原牌|七圣/.test(e.msg)) {
		if (!name) {
			let list = gsCfg.getfileYaml(`${_path}/${plugin}/resources/Atlas_alias/`, 'yuanmo_tujian')
			name = info_img(e, list, msg)
		}
...
const filePath = async function (e) {
	...
	for (let [index, val] of data.entries()) {
      ...
      if(val == "yuanmo_tujian") {
        e.reply(segment.image(`file:///${pathPlus}/enemy/${msg}.png`));
      }
			return true;
		}
	}
...
const send_Msg = async function (e, type, name) {
	...
	if (type == "all") {
		for (let [index, val] of list.entries()) {
			...
			let new_name = info_img(e, gsCfg.getfileYaml(`${_path}/${plugin}/resources/Atlas_alias/`,
				val), name)
			if (new_name) {
				name = new_name
				type = val;
...
async function Atlas_list(e) {
	let list = gsCfg.getfileYaml(`${_path}/${plugin}/resources/Atlas_alias/`, 'Atlas_list')
	let name = e.msg.replace(/#|井/g, "")
	for (let i in list) {
		let title = i.split("|");
		for (let j = 0; j < title.length; j++) {
			if (title[j] == name) {
        e.reply([`当前选择【${name}】`, "请选择:\n" + list[i].join("\n")])
