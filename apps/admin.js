...
const rodom = async function() {
	var image = fs.readdirSync(`${common.getPluginsPath()}/${plugin}/resources/admin/imgs/bg`);
	var list_img = [];
...
async function updateRes(e) {
	...
	if (fs.existsSync(`${resPath}/xiaoyao-plus/`)) {
		...
		exec(command, function(error, stdout, stdearr) {
			if (error) {
				e.reply("角色图片加量包安装失败！\nError code: " + error.code + "\n" + error.stack + "\n 请稍后重试。");
			} else {
