...
//#体力
async function Note(e, {render}, poke) {
	...
	if (isV3) {
		...
		let MysInfo = await import(`file://${common.getPluginsPath()}/genshin/model/mys/mysInfo.js`);
		...
		if (res.retcode != 0) {
			if (res.message == "Please login") {
				...
				logger.info(`体力查询错误:${JSON.stringify(res)}`);
			}
	...
	for (let item in notes.noteCfg) {
		...
		for (let i of taskUser) {
			...
			if (isV3) {
				let {
					getRender
				} = await import(`file://${_path}/plugins/xiaoyao-plugin/adapter/render.js`);
...
async function pokeNote(e, {render}) {
...
async function Note_appoint(e) {
	let mbPath = `${_path}/plugins/xiaoyao-plugin/resources/dailyNote/`;
...
const note_file = function (xiaoyao) {
	let url1 = `./plugins/xiaoyao-plugin/resources/dailyNote/Template/`
	let url2 = `./plugins/xiaoyao-plugin/resources/BJT-Templet/` //冤种情况。。
	let url3 = `./plugins/xiaoyao-plugin/resources/dailyNote/background_image/`
